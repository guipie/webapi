...
namespace Monster.Core.Controllers.Basic
{
    public class BaseController<IServiceBase> : Controller
    {
        ...
        [ApiExplorerSettings(IgnoreApi = true)]
        public virtual async Task<ActionResult> GetOneResponse(object id)
        {
            string data = await Task.FromResult(InvokeService("FindOne", new object[] { id }).Serialize());
            return Content(data);
        }
        [ApiExplorerSettings(IgnoreApi = true)]
        public virtual async Task<ActionResult> GetOneEntity(int id)
        {
            string data = await Task.FromResult(InvokeService("GetOne", new object[] { id }).Serialize());
            return Content(data);
        }
        [ApiExplorerSettings(IgnoreApi = true)]
