...
namespace Monster.Core.Utilities
{
    public class CrawlerService : ICrawlerService
    {
        ...
        public async Task<string> Start(Uri uri, Encoding coding = null, string proxy = null)
        {
            ...
            return await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    request.Timeout = 60 * 1000;//定义请求超时时间为5秒
                    request.KeepAlive = true;//启用长连接
                    request.Method = "GET";//定义请求方式为GET              
                    if (proxy != null)
                    {
                        request.Proxy = new WebProxy(proxy, 80);//设置代理服务器IP，伪装请求地址
                    }
                    ...
                    {//获取请求响应
                        ...
                        if (!response.ContentEncoding.IsNullOrEmpty() && response.ContentEncoding.ToLower().Contains("gzip"))//解压
                        {
                            using GZipStream stream = new GZipStream(response.GetResponseStream(), CompressionMode.Decompress);
                            using StreamReader reader = new StreamReader(stream, coding);
                            pageSource = reader.ReadToEnd();
                        }
                        else if (!response.ContentEncoding.IsNullOrEmpty() && response.ContentEncoding.ToLower().Contains("deflate"))//解压
                        {
